...
@section('content')
    <x-card class="shadow-md max-w-xl">
        ...
        <form method="POST" action="{{ route('customers.update', $customer) }}" class="space-y-4">
            ...
            @method('PUT')
            <div>
                <label class="block text-sm font-medium text-text mb-1">Kode Customer</label>
                <input type="text" value="{{ $customer->kode_customer }}" disabled
            ...
            <div>
                ...
                <textarea name="alamat"
                    class="w-full px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary">{{ old('alamat', $customer->alamat) }}</textarea>
            </div>
