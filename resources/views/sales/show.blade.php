...
@section('content')
    ...
    <x-card class="shadow-md mb-6">
        <div class="flex items-start justify-between mb-4">
            <div>
                <p class="text-lg font-semibold text-text">{{ $sale->nomor_transaksi }}</p>
                <p class="text-sm text-slate-500">{{ $sale->customer->nama }} — {{ $sale->tanggal->format('d M Y') }}</p>
            </div>
            <span
                class="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-accent capitalize">{{ $sale->tipe }}</span>
        </div>
            ...
            <tbody class="divide-y divide-slate-100">
                @foreach ($sale->items as $item)
                    <tr>
                        ...
                        <td class="px-3 py-2 text-right">{{ number_format($item->subtotal_hpp, 2) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="grid grid-cols-2 gap-4 text-sm">
            <div class="space-y-1">
                <div class="flex justify-between"><span
                        class="text-slate-500">Subtotal</span><span>{{ number_format($sale->subtotal, 2) }}</span></div>
                <div class="flex justify-between"><span class="text-slate-500">Diskon
                        Dagang</span><span>{{ number_format($sale->diskon_dagang, 2) }}</span></div>
                <div class="flex justify-between"><span class="text-slate-500">DPP
                        PPN</span><span>{{ number_format($sale->dpp_ppn, 2) }}</span></div>
                <div class="flex justify-between"><span
                        class="text-slate-500">PPN</span><span>{{ number_format($sale->ppn, 2) }}</span></div>
                <div class="flex justify-between font-semibold border-t border-slate-200 pt-1">
                    <span>Total</span><span>{{ number_format($sale->total, 2) }}</span></div>
            </div>
            @if ($sale->receivable)
                <div class="bg-amber-50 rounded-lg p-3">
                    <p class="text-xs text-slate-500">Piutang Terkait</p>
                    <a href="{{ route('receivables.show', $sale->receivable) }}"
                        class="text-primary font-medium">{{ $sale->receivable->nomor_invoice }} — Sisa
                        {{ number_format($sale->receivable->sisa_piutang, 2) }}</a>
                </div>
            @endif
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50 text-left text-slate-500">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right">HPP Retur</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @foreach ($sale->returns as $return)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right">{{ number_format($return->hpp_retur, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
