...
@section('content')
    <x-card class="shadow-md">
        ...
        <p class="text-sm text-slate-500 mb-4">Retur untuk {{ $sale->nomor_transaksi }}</p>

        <form method="POST" action="{{ route('sales.return.store', $sale) }}" class="space-y-4">
            @csrf
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50 text-left text-slate-500">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-right">Qty Dibeli</th>
                        <th class="px-3 py-2 w-40">Qty Retur</th>
                    </tr>
                </thead>
                    ...
                    @foreach ($sale->items as $item)
                        <tr>
                            <td class="px-3 py-2">
                                ...
                                <input type="hidden" name="items[{{ $loop->index }}][sale_item_id]"
                                    value="{{ $item->id }}">
                            </td>
